...
fn benchmark_recovery_rate() {
    ...
    if !corpus_dir.exists() {
        eprintln!(
            "Nemetz corpus not found at {:?}. Run 'git lfs pull' first.",
            corpus_dir
        );
        return;
    }
    ...
    eprintln!(
        "Nemetz corpus: {} DBs, {} live records, {} total recovered",
...
fn anti_forensic_corpus_no_crash() {
    ...
    for entry in std::fs::read_dir(&corpus_dir).unwrap() {
        ...
        if path.extension().is_some_and(|e| e == "db") {
            ...
            let _ = ForensicEngine::new(&db, None).map(|e| e.recover_all());
        }
    }
}
...
fn generate_test_dbs() -> Vec<(&'static str, Vec<u8>)> {
    ...
    // DB with deletions
    {
        ...
        let conn = rusqlite::Connection::open(&path).unwrap();
        conn.execute_batch("PRAGMA secure_delete=OFF; PRAGMA page_size=1024;")
            .unwrap();
        conn.execute_batch(
