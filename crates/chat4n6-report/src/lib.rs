...
mod tests {
    ...
    fn timeline_sorted_by_timestamp() {
        ...
        let mut result = make_test_result();
        result.chats = vec![chat_a, chat_b];

        let out = TempDir::new().unwrap();
        let gen = ReportGenerator::new().unwrap();
        gen.render("TimelineTest", &result, out.path()).unwrap();

        assert!(
            out.path().join("timeline.html").exists(),
            "timeline.html must be generated by render()"
        );
        ...
        assert!(
            pos_alpha < pos_later,
            "alpha message must appear before alpha later in timeline"
        );
    }
